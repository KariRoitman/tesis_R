...
Espectra.Orig <- Espectra.Orig %>%
  distinct(spot.a.1, .keep_all = TRUE)

Base.Union.Clinc.Covid <- Base.Union.Clinc.Covid %>%
  distinct(spot.a.1, .keep_all = TRUE)
...
# Redondear nombres de columnas a 2 decimales, por ejemplo
...
data_freq_todos_grafico <- as.data.frame(table(Datos_actualizados$institucion, Datos_actualizados$PCR.Cov))
colnames(data_freq_todos_grafico) <- c("institucion", "covid_status", "frecuencia")

data_freq_todos_grafico<-ggplot(data_freq_todos_grafico, aes(x = institucion, y = frecuencia, fill = covid_status)) +
  geom_bar(stat = "identity", position = "stack") +
  geom_text(aes(label = frecuencia),
            position = position_stack(vjust = 0.5), # centra el texto en cada bloque
            color = "white", size = 4) +
  labs(title = "Espectros por Institución",
       x = "Institución",
       y = "Frecuencia") +
...
```
```{r}
save(data_freq_todos_grafico, file = "data_freq_todos_grafico.rda")
```

```{r}

print(data_freq_todos_grafico)
```
...
p_with_density_todos <- ggMarginal(p, type = "density", margins = "both", groupColour = TRUE, groupFill = TRUE)

# Mostrar el gráfico final
print(p_with_density_todos)
```

```{r}
save(p_with_density_todos, file = "p_with_density_todos.rda")
```
...
p <- ggplot(pca_coor, aes(x = PC2, y = PC3, color = pca.before$equipo, shape = pca.before$covid)) +
  geom_point(size = 2) +  # Tamaño de los puntos
  labs(title = 'PCA con mixOmics-Institución y estartus COVID',
...
# Añadir gráficos de densidad marginal con ggExtra
p_with_density2_todos <- ggMarginal(p, type = "density", margins = "both", groupColour = TRUE, groupFill = TRUE)

# Mostrar el gráfico final
print(p_with_density2_todos)
```


```{r}
save(p_with_density2_todos, file = "p_with_density2_todos.rda")
```
...
p <- ggplot(pca_coor, aes(x = PC1, y = PC2, color = pca.before$covid)) +
  geom_point(size = 2) +  # Tamaño de los puntos
  labs(title = 'PCA con mixOmics - Estatus COVID',
       x = 'Componente principal 1',
       y = 'Componente principal 2',
       color = 'Batch') +
  theme_minimal() +
...
# Añadir gráficos de densidad marginal con ggExtra
p_with_density3_todos <- ggMarginal(p, type = "density", margins = "both", groupColour = TRUE, groupFill = TRUE)

# Mostrar el gráfico final
print(p_with_density3_todos)
```



```{r}
save(p_with_density3_todos, file = "p_with_density3_todos.rda")
```
...
library(ggplot2)
explained_var_plot_MixOmics_todos<- ggplot(var_df, aes(x = Component, y = ExplainedVariance)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  geom_text(aes(label = round(ExplainedVariance, 1)), vjust = -0.5, size = 3) +

  labs(
    title = "Varianza explicada por componente prinicipal-MixOmics",
...
print(explained_var_plot_MixOmics_todos)

```

```{r}
save(explained_var_plot_MixOmics_todos, file = "explained_var_plot_MixOmics_todos.rda")
```
...
# Graficar usando ggplot2
library(ggplot2)
porcentaje_varianza_explicada_prcomp_todos<- ggplot(var_df_10, aes(x = Component, y = ExplainedVariance)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  geom_text(aes(label = round(ExplainedVariance, 1)), vjust = -0.5, size = 3) +
    ylim(0, max(var_df_10$ExplainedVariance) * 1.1) +
  labs(
    title = "Varianza explicada de cada componente principal- Libreria prcomp",
    x = "Componente Principal",
    y = "Varianza Explicada (%)"
  ) +
  theme_minimal()+
...
print(porcentaje_varianza_explicada_prcomp_todos)
```



```{r}
save(porcentaje_varianza_explicada_prcomp_todos, file = "porcentaje_varianza_explicada_prcomp_todos.rda")
```
...
# Graficar usando ggplot2
library(ggplot2)
exaplained_var_plot_rbase_todos<- ggplot(var_df_10, aes(x = Component, y = ExplainedVariance)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  geom_text(aes(label = round(ExplainedVariance, 1)), vjust = -0.5, size = 3) +
    ylim(0, max(var_df_10$ExplainedVariance) * 1.1) +
  labs(
...
print(exaplained_var_plot_rbase_todos)
```
...
pca_prcomp_todos_equipo<-plot_pca(pca_data, PC1, PC2, color_by = "equipo", shape_by = "covid",
         title = "PCA - PC1 y PC2 por equipo", usar_paleta_auto = TRUE)
```


```{r}
save(pca_prcomp_todos_equipo, file = "pca_prcomp_todos_equipo.rda")
```


```{r}
pca_prcomp_todos2<- plot_pca(pca_data, PC2, PC3, color_by = "equipo", shape_by = "covid", 
         title = "PCA - PC2 vs PC3 por equipo y pos/neg", usar_paleta_auto = TRUE)
```


```{r}
save(pca_prcomp_todos2, file = "pca_prcomp_todos2.rda")
```


```{r}
pca_prcomp_todos_fecha<- plot_pca(pca_data, PC1, PC2, color_by = "fecha", shape_by = "covid",
...
extra_top    <- 0.30     # aire arriba (en unidades de PC2)
extra_bottom <- 0.40     # aire abajo

pca_prcomp_todos_fecha <- plot_pca(
  pca_data, PC1, PC2, color_by = "fecha", shape_by = "covid",
  title = "PCA - PC1 vs PC2 por fecha", usar_paleta_auto = TRUE
) +
  # Solo agregamos aire con 'add': no mueve el centro ni recorta datos
  scale_y_continuous(expand = expansion(mult = 0, add = c(extra_bottom, extra_top))) +
  scale_x_continuous(expand = expansion(mult = 0.02)) +
  theme(
    plot.margin = margin(t = 12, r = 10, b = 18, l = 10)
  ) +
  # Compactar y acomodar la leyenda de 'fecha' en 2 columnas para que no se corte
  guides(
    colour = guide_legend(ncol = 2, byrow = TRUE, title.position = "top"),
    shape  = guide_legend(order = 1) # la de covid queda arriba
  ) +
  theme(
...
```{r}
save(pca_prcomp_todos_fecha, file = "pca_prcomp_todos_fecha.rda")
```


```{r}
pca_prcomp_todos<-plot_pca(pca_data, PC1, PC2, color_by = "covid", shape_by = "covid", 
...
g1 <- pca_prcomp_todos_equipo + guides(color = "none", shape = "none")
g2 <- pca_prcomp_todos_fecha  + guides(color = "none", shape = "none")
g3 <- pca_prcomp_todos + 
  labs(color = "covid", shape = "covid") +
  theme(legend.position = "bottom")
g4 <- exaplained_var_plot_rbase_todos + theme(legend.position = "none")

todos_pca <- (g1 | g2) / (g3 | g4) + plot_layout(guides = "keep")

# ↓↓↓ aplica a TODOS los subplots, y gana prioridad
todos_pca <- todos_pca & theme(
  plot.title  = element_text(size = 10, face = "bold"),  # -2/3 pts
...
```{r}
save(todos_pca, file = "todos_pca.rda")
```
-----------------------------------------------------------------------------------------
...
```{r}
UMAP_equipo_todos<-plot_umap(umap_df, V1, V2, color_by = "equipo", shape_by = "covid",
...
UMAP_equipo_todos<-plot_umap(umap_df, V1, V2, color_by = "equipo", shape_by = "covid",
          title = "UMAP Discriminado por equipo", usar_paleta_auto = TRUE) +
  # Solo agregamos aire con 'add': no mueve el centro ni recorta datos
  scale_y_continuous(expand = expansion(mult = 0, add = c(extra_bottom, extra_top))) +
  scale_x_continuous(expand = expansion(mult = 0.02)) +
  theme(
...
UMAP_fecha_todos<-plot_umap(umap_df, V1, V2, color_by = "fecha", shape_by = "covid",
          title = "UMAP Discriminado por fecha", usar_paleta_auto = TRUE) +
  # Solo agregamos aire con 'add': no mueve el centro ni recorta datos
  scale_y_continuous(expand = expansion(mult = 0, add = c(extra_bottom, extra_top))) +
  scale_x_continuous(expand = expansion(mult = 0.02)) +
  theme(
...
# A <- UMAP_equipo_todos +
#   coord_equal() +
#   labs(color = "equipo", shape = "covid") +
#   guides(
#     color = guide_legend(ncol = 2, byrow = TRUE, override.aes = list(size = 2)),
#     shape = guide_legend(nrow = 1)
#   ) 
# 
# B <- UMAP_fecha_todos +
#   coord_equal() +
#   labs(color = "fecha", shape = "covid") +
#   guides(
#     color = guide_legend(ncol = 2, byrow = TRUE, override.aes = list(size = 2)),
#     shape = guide_legend(nrow = 1)
#   ) 
# 
# combined_plot_UMAP_todos <- (A + B) +
#   plot_annotation(title = "UMAP-Reducción no lineal de las dimensiones", tag_levels = "A") +
...
#     legend.key.size = unit(3, "mm"),   # claves más pequeñas
#     legend.spacing.y = unit(1, "mm")
#   )
# 
# print(combined_plot_UMAP_todos)
...
# Tamaños (podés ajustarlos rápido desde acá)
sz_title_main  <- 20   # título general (arriba)
sz_title_panel <- 10   # títulos "UMAP Discriminado por..."
sz_axis_title  <- 4    # "V1", "V2"
sz_axis_text   <- 4    # ticks de ejes
sz_leg_title   <- 8    # títulos de leyenda
sz_leg_text    <- 8    # items de leyenda
sz_leg_key_mm  <- 2    # tamaño de clave en mm
...
combined_plot_UMAP_todos <-
  (A + B) +
  plot_annotation(
    title = "UMAP – Reducción no lineal de las dimensiones",
    theme = theme(plot.title = element_text(size = sz_title_main, face = "bold"))
  ) +
  plot_layout(guides = "collect") &
  theme(
    # Leyendas a la derecha y chiquitas
    legend.position = "right",
    legend.box = "vertical",
    legend.title = element_text(size = sz_leg_title),
    legend.text  = element_text(size = sz_leg_text),
    legend.key.size = unit(sz_leg_key_mm, "mm"),

    # Títulos de cada panel más grandes
    plot.title = element_text(size = sz_title_panel, face = "bold"),
...
```{r}
save(combined_plot_UMAP_todos, file = "combined_plot_UMAP_todos.rda")
```
...
umap_coor2$fecha <- Datos_actualizados$fecha
# Graficar UMAP con ggplot2
library(ggplot2)
UMAP_ajustado_equipo<- ggplot(umap_coor2, aes(x = V1, y = V2, color = equipo, shape = covid)) +
  geom_point(size = 2) +
  labs(title = "UMAP - Discriminado por fecha",
       x = "UMAP1",
       y = "UMAP2") +
  theme_minimal()+
  theme(
...
print(UMAP_ajustado_equipo)

library(ggplot2)
UMAP_ajustado_fecha<- ggplot(umap_coor2, aes(x = V1, y = V2, color = fecha, shape = covid)) +
  geom_point(size = 2) +
  labs(title = "UMAP - Discriminado por fecha",
       x = "UMAP1",
       y = "UMAP2") +
...
print(UMAP_ajustado_fecha)
```
```{r}
# 
# A <- UMAP_ajustado_equipo  +
#   coord_equal() +
#   labs(color = "equipo", shape = "covid") +
#   guides(
#     color = guide_legend(ncol = 2, byrow = TRUE, override.aes = list(size = 2)),
...
#   )
# 
# combined_plot_UMAP_ajustado_todos <- (A + B) +
#   plot_annotation(title = "UMAP parámetros ajustados", tag_levels = "A") +
#   plot_layout(guides = "collect") &
#   theme(
#     legend.position = "right",
#     legend.box = "vertical",
...
#   )
# 
# print(combined_plot_UMAP_ajustado_todos)
...
combined_plot_UMAP_todos_ajustado <-
  (A + B) +
  plot_annotation(
    title = "UMAP ajustado– Reducción no lineal de las dimensiones",
    theme = theme(plot.title = element_text(size = sz_title_main, face = "bold"))
  ) +
  plot_layout(guides = "collect") &
  theme(
...
print(combined_plot_UMAP_todos_ajustado)
```

```{r}
save(combined_plot_UMAP_todos_ajustado, file= "combined_plot_UMAP_todos_ajustado.rda")
```
...
g2 <- plot_tsne(tsne_data, Dim1, Dim2, color_by = "fecha", shape_by = "covid", 
                title = "t-SNE 1 y 2", usar_paleta_auto = TRUE)

# Combinarlos con patchwork
combined_plot_tsne_equipo_tot <- (g1 | g2) +
  plot_annotation(
...
# Mostrar
print(combined_plot_tsne_equipo_tot)
...
# Crear gráficos con la función
g1 <- plot_tsne(tsne_data, Dim1, Dim2, color_by = "equipo", shape_by = "covid", 
                title = "t-SNE-Discriminado por equipo", usar_paleta_auto = TRUE) + coord_equal() +
  labs(color = "equipo", shape = "covid") +  coord_cartesian(expand = FALSE)
  guides(
    color = guide_legend(ncol = 2, byrow = TRUE, override.aes = list(size = 2)),
    shape = guide_legend(nrow = 1)
  )
...
g2 <- plot_tsne(tsne_data, Dim1, Dim2, color_by = "fecha", shape_by = "covid", 
                title = "t-SNE-Discriminado por fecha", usar_paleta_auto = TRUE)+ coord_equal() +
  labs(color = "equipo", shape = "covid") + coord_cartesian(expand = FALSE)
  guides(
    color = guide_legend(ncol = 2, byrow = TRUE, override.aes = list(size = 2)),
    shape = guide_legend(nrow = 1)
...
combined_plot_tsne_todos <- (g1 + g2) +
  plot_annotation(title = "t-SNE -Técnica no lineal de reducción de la dimensionalidad", tag_levels = "A") +
  plot_layout(guides = "collect") &
  theme(
    plot.title = element_text(size = 12),  # títulos de cada panel
    legend.position = "right",
    legend.box = "vertical",
    legend.title = element_text(size = 8),
    legend.text  = element_text(size = 7),
...
print(combined_plot_tsne_todos)


```

```{r}
save(combined_plot_tsne_todos, file = "combined_plot_tsne_todos.rda")
...
para detectar el peso de cada factor en el efecto batch. Propósito: Evaluar si las puntuaciones del PCA difieren significativamente entre los niveles de un factor (como PCR.Cov, fecha o equipo). Kruskal-Wallis es una prueba no paramétrica que no asume normalidad, lo que la hace adecuada si los datos no pasan la prueba de Shapiro-Wilk.
...
combined_plot_kruskal_tot <- combined_plot + 
  plot_annotation(
...
print(combined_plot_kruskal_tot)
```


```{r}
save(combined_plot_kruskal_tot, file = "combined_plot_kruskal_tot.rda")
...
COVID<-Datos_actualizados$PCR.Cov
plot_dendrogram(featureMatrix_num, COVID,
                titulo = "Dendrograma por resultado COVID",
                nombre_png = "dendrograma_COVID.png")




# Clustering por equipo en positivos
equipo_covidpos <- Datos_actualizados$equipo[indices_cov_pos]
indices_cov_pos <- Datos_actualizados$PCR.Cov == "Cov.Pos"
plot_dendrogram(featureMatrix_num[indices_cov_pos, ],
                equipo_covidpos,
                titulo = "Clustering Positivos por Equipo",
                nombre_png = "dendrograma_COVID.pos.png")


equipo_covidneg <- Datos_actualizados$equipo[indices_cov_neg]
...
plot_dendrogram(featureMatrix_num, fecha,
                ...
                nombre_png = "dendrograma_fecha.png",
                              palette_type = "brewer")
...
library(ggplotify)  # para as.ggplot()
library(patchwork)
...
# A: COVID
g_covid <- as.ggplot(function(){
  par(mar = c(3, 3, 2, 8), xpd = NA)  # margen derecho grande
  plot_dendrogram(featureMatrix_num, COVID,
                  titulo = "Dendrograma por COVID",
                  nombre_png = NULL)
  # achicar leyenda que dibuja tu función
  par(cex = 0.7)
})

# B: equipo
g_equipo <- as.ggplot(function(){
  par(mar = c(3, 3, 2, 8), xpd = NA)
  plot_dendrogram(featureMatrix_num, Equipo,
                  titulo = "Clustering por equipo",
                  nombre_png = NULL)
  par(cex = 0.7)
})

# C: fecha (muchas categorías → más margen inferior)
g_fecha <- as.ggplot(function(){
  par(mar = c(6, 3, 2, 2), xpd = NA)
  plot_dendrogram(featureMatrix_num, Fecha,
                  titulo = "Clustering por fecha",
                  nombre_png = NULL, palette_type = "brewer")
  par(cex = 0.5)
})
...
save(g_equipo, file = "g_fecha.rda")
save(g_fecha, file = "g_equipo.rda")
save(g_covid, file = "g_covid.rda")
...
# Plot PCA después de la corrección
plot(pca_after$x[,1:2], col = col_vector, main = "PCA base Post Combar por equipo")
legend("topright", 
       legend = levels(batch_colors), 
       col = palette_colors, 
       pch = 16, 
...
pca_data <- as.data.frame(pca_after$x)
pca_data$fecha <- Datos_actualizados$fecha
pca_data$equipo <- Datos_actualizados$equipo
pca_data$covid<- Datos_actualizados$PCR.Cov
# Gráficos PCA usando la función
pca_prcomp_todos_batch_equipo<-plot_pca(pca_data, PC1, PC2, color_by = "equipo", shape_by = "covid",
         title = "PCA - Efecto Batch por equipo", usar_paleta_auto = TRUE)


pca_prcomp_todos_batch_equipo2<-plot_pca(pca_data, PC2, PC3, color_by = "equipo", shape_by = "covid",
         title = "PCA - Efecto Batch por equipo", usar_paleta_auto = TRUE)

pca_prcomp_todos_batch_equipo3<-plot_pca(pca_data, PC1, PC3, color_by = "equipo", shape_by = "covid",
         title = "PCA - Efecto Batch por equipo", usar_paleta_auto = TRUE)
...
porcentaje_varianza_explicada <- (pca_after$sdev^2 / sum(pca_after$sdev^2)) * 100
components <- seq_along(porcentaje_varianza_explicada) 
...
# Graficar usando ggplot2
library(ggplot2)
batch_varianza_explicada_prcomp_todos<- ggplot(var_df_10, aes(x = Component, y = ExplainedVariance)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  geom_text(aes(label = round(ExplainedVariance, 1)), vjust = -0.5, size = 3) +
    ylim(0, max(var_df_10$ExplainedVariance) * 1.1) +
  labs(
    title = "Varianza explicada de cada componente",
    x = "Componente Principal",
    y = "Varianza Explicada (%)"
  ) +
  theme_minimal()+
...
print(batch_varianza_explicada_prcomp_todos)
...
g1 <- pca_prcomp_todos_batch_equipo + guides(color = "none", shape = "none")
g2 <- pca_prcomp_todos_batch_equipo2  + guides(color = "none", shape = "none")
g4 <- batch_varianza_explicada_prcomp_todos + 
  labs(color = "covid", shape = "covid") +
  theme(legend.position = "bottom")
g3 <- pca_prcomp_todos_batch_equipo3 + theme(legend.position = "none")+ guides(color = "none", shape = "none")

todos_pca_batch_equipo <- (g1 | g2) / (g3 | g4 ) + plot_layout(guides = "keep")

# ↓↓↓ aplica a TODOS los subplots, y gana prioridad
todos_pca_batch_equipo <- todos_pca_batch_equipo & theme(
  plot.title  = element_text(size = 10, face = "bold"),  # -2/3 pts
  axis.title.x = element_text(size = 9),
  axis.title.y = element_text(size = 9),
  axis.text.x  = element_text(size = 8),
...
```
```{r}
save(todos_pca_batch_equipo, file = "todos_pca_batch_equipo.rda")
...
pca_kruskal_COMBAT_todos_equipo<- ggplot(factor_summary, aes(x = reorder(Factor, MeanLogPValue), y = MeanLogPValue, fill = Factor)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  theme_minimal() +
  labs(title = "Importancia de los Factores -PCA",
...
print(pca_kruskal_COMBAT_todos_equipo)
```
...
```{r}
save(pca_kruskal_COMBAT_todos_equipo, file = "pca_kruskal_COMBAT_todos_equipo.rda")
...
umap_coor_combat$fecha <- Datos_actualizados$fecha
# Graficar UMAP con ggplot2
library(ggplot2)
UMAP_postCombat_equipo<-ggplot(umap_coor_combat, aes(x = UMAP1, y = UMAP2, color = equipo, shape = covid)) +
  geom_point(size = 3) +
  labs(title = "UMAP equipo-Post Combat equipo",
       x = "UMAP1",
       y = "UMAP2") +
  theme_minimal()


UMAP_postCombat_fecha<-ggplot(umap_coor_combat, aes(x = UMAP1, y = UMAP2, color = fecha, shape = covid)) +
  geom_point(size = 3) +
  labs(title = "UMAP fecha-Post Combat equipo",
       x = "UMAP1",
       y = "UMAP2") +
  theme_minimal()
```
...
A <- UMAP_postCombat_equipo +
  coord_equal() +
  labs(color = "equipo", shape = "covid")

B <- UMAP_postCombat_fecha +
  coord_equal() +
  labs(color = "fecha", shape = "covid")
...
print(combined_plot_UMAP_combat_todos)
```

```{r}
save(combined_plot_UMAP_combat_todos, file = "combined_plot_UMAP_combat_todos.rda")
```
...
# 0) Chequeos rápidos
stopifnot(nrow(umap_scores) == nrow(Datos_actualizados))
Datos_actualizados <- Datos_actualizados %>%
  mutate(
    fecha  = as.factor(fecha),
    PCR.Cov = as.factor(PCR.Cov),
    equipo = as.factor(equipo)
  )

# 1) Función Kruskal por cada dimensión de UMAP
kruskal_umap <- function(umap_scores, metadata, factor) {
  res <- vector("list", ncol(umap_scores))
  for (i in seq_len(ncol(umap_scores))) {
    umap_component <- umap_scores[, i]
    # kruskal requiere un factor/categórica al agrupar
    grp <- as.factor(metadata[[factor]])
    # manejar NA en el factor
    keep <- !is.na(grp) & !is.na(umap_component)
    model <- kruskal.test(umap_component[keep] ~ grp[keep])
    res[[i]] <- data.frame(UMAP_Dim = i, Factor = factor, PValue = model$p.value)
  }
  do.call(rbind, res)
}

# 2) Ejecutar para cada factor (solo los que existan)
factors <- c("PCR.Cov", "fecha", "equipo")
factors <- intersect(factors, colnames(Datos_actualizados))

kruskal_results_umap <- do.call(
  rbind,
  lapply(factors, function(f) kruskal_umap(umap_scores, Datos_actualizados, f))
)

# 3) Resumen robusto (evitar -log10(0))
eps <- .Machine$double.xmin
factor_summary_umap <- kruskal_results_umap %>%
  group_by(Factor) %>%
  summarise(
    MeanLogPValue = -log10(mean(pmax(PValue, eps), na.rm = TRUE)),
    .groups = "drop"
  )

# 4) Gráfico (sin '+' colgando)
umap_postcombat_var_todos <- ggplot(factor_summary_umap,
                                    aes(x = reorder(Factor, MeanLogPValue),
                                        y = MeanLogPValue, fill = Factor)) +
  geom_col() +
  coord_flip() +
  theme_minimal() +
  labs(title = "Importancia de factores UMAP",
       x = "Factor",
       y = "-log10(P-valor medio)",
       fill = "Factor")

print(umap_postcombat_var_todos)
...
combined_var_combat <- (
  pca_kruskal_COMBAT_todos_equipo + umap_postcombat_var_todos
)

# Agregamos los títulos y anotaciones
combined_var_combat_todos <- combined_var_combat + 
  plot_annotation(
    title = "Influencia de los factores en los datos post Combat equipo"
    #tag_levels = "A"  # Etiquetas A, B, C, ...
  ) + 
  plot_layout(guides = "collect", widths = c(1, 1))  # Para asegurar que ambos gráficos tengan el mismo ancho

# Para ver el gráfico
print(combined_var_combat_todos)


```

```{r}
save(combined_var_combat_todos, file = "combined_var_combat_todos.rda")
```
...
# Dibuja el dendrograma
plot(dend, main = "Dendrograma por resultado COVID- Post combat por equipo")
legend("topright",           # Posición de la leyenda
       legend = c("COVID Negativo", "COVID Positivo"), # Textos de la leyenda
       fill = c("red", "blue"),                        # Colores de la leyenda
       border = "black",                               # Borde de las cajas de color
...
grid.echo()
g <- grid.grab()
cluster_covidcombat_todos<- ggplotify::as.ggplot(g)
```
...
# Dibujás en base R
plot(dend_eq, main = "Clustering Diferenciado por Equipo- post ComBat por equipo")
legend("topright", legend = levels(equipo_factor), col = palette, pch = 19, title = "Equipo")

# Lo convertís a grob y luego a ggplot
grid.echo()
g <- grid.grab()
cluster_equipocombat_todos <- ggplotify::as.ggplot(g)  # <- ahora es un ggplot
```
...
plot(dend_fecha, main = "Clustering Diferenciado por Fecha-Post Combat por equipo")
legend(
  "topright",
...
grid.echo()
g <- grid.grab()
cluster_fechacombat_todos <- ggplotify::as.ggplot(g)  # <- ahora es un ggplot
...
```{r}
save(cluster_equipocombat_todos, file= "cluster_equipocombat_todos.rda")
save(cluster_fechacombat_todos, file= "cluster_fechacombat_todos.rda")
save(cluster_covidcombat_todos, file= "cluster_covidcombat_todos.rda")
```
...
# Definir colores manualmente (puedes cambiar los nombres de niveles y colores)
niveles <- levels(batch_colors)
col_vector <- setNames(
  sample(colors(), length(niveles)),  # elige colores aleatorios de la paleta base R
  niveles
)
```
...
```{r}
pca_after <- prcomp(combat_corrected_fecha, scale. = TRUE)


png("PCA post COMBAT fecha.png", width = 800, height = 600)
# Plot PCA después de la corrección
plot(pca_after$x[,1:2], col = col_vector, main = "PCA post COMBAT fecha")
legend("topright", 
       legend = levels(batch_colors), 
       col = palette_colors, 
       pch = 16, 
...
# Plot PCA después de la corrección
plot(pca_after$x[,1:2], col = col_vector, main = "PCA base Post Combat por fecha")
legend("topright", 
       legend = levels(batch_colors), 
       col = palette_colors, 
       pch = 16, 
...
pca_data$covid<- Datos_actualizados$PCR.Cov
# Gráficos PCA usando la función
pca_prcomp_todos_batch_fecha<-plot_pca(pca_data, PC1, PC2, color_by = "fecha", shape_by = "covid",
         title = "PCA - Post Combat Fecha", usar_paleta_auto = TRUE)


pca_prcomp_todos_batch_fecha2<-plot_pca(pca_data, PC2, PC3, color_by = "fecha", shape_by = "covid",
         title = "PCA - Post Combat Fecha", usar_paleta_auto = TRUE)

pca_prcomp_todos_batch_fecha3<-plot_pca(pca_data, PC1, PC3, color_by = "fecha", shape_by = "covid",
         title = "PCA - Post Combat Fecha", usar_paleta_auto = TRUE)
```
...
#     legend.text = element_text(size = 10)  # Reducir tamaño del texto de la leyenda
#   )
# print(batch_varianza_explicada_prcomp_todos_fecha)



porcentaje_varianza_explicada <- (pca_after$sdev^2 / sum(pca_after$sdev^2)) * 100
var_df <- data.frame(
  Component = seq_along(porcentaje_varianza_explicada),
  ExplainedVariance = porcentaje_varianza_explicada
)
var_df_10 <- head(var_df, 10)

library(ggplot2)

batchfecha_varianza_explicada_prcomp_todos <- ggplot(var_df_10, aes(x = Component, y = ExplainedVariance)) +
  geom_col() +
  geom_text(aes(label = round(ExplainedVariance, 1)), vjust = -0.4, size = 3) +
  # evita recortes arriba sin forzar límites fijos
  scale_y_continuous(expand = expansion(mult = c(0, 0.1))) +
  scale_x_continuous(breaks = var_df_10$Component,
                     labels = paste0("PC", var_df_10$Component)) +
  labs(
    title = "Varianza explicada por componente (prcomp)",
    x = "Componente principal",
    y = "Varianza explicada (%)"
  ) +
  theme_minimal() +
...
# Imprimir el objeto correcto
print(batchfecha_varianza_explicada_prcomp_todos)
```
...
g1 <- pca_prcomp_todos_batch_fecha + guides(color = "none", shape = "none")
g2 <- pca_prcomp_todos_batch_fecha2  + guides(color = "none", shape = "none")
g4 <- batchfecha_varianza_explicada_prcomp_todos + 
  labs(color = "covid", shape = "covid") +
  theme(legend.position = "bottom")
g3 <- pca_prcomp_todos_batch_fecha3 + theme(legend.position = "none")+ guides(color = "none", shape = "none")

todos_pca_batch_equipo <- (g1 | g2) / (g3 | g4 ) + plot_layout(guides = "keep")

# ↓↓↓ aplica a TODOS los subplots, y gana prioridad
todos_pca_batch_fecha <- todos_pca_batch_equipo & theme(
  plot.title  = element_text(size = 10, face = "bold"),  # -2/3 pts
  axis.title.x = element_text(size = 9),
  axis.title.y = element_text(size = 9),
  axis.text.x  = element_text(size = 8),
...
```{r}
save(todos_pca_batch_fecha, file = "todos_pca_batch_fecha.rda")
```
...
# Ajustar parámetros directamente en la función umap
umap_combat <- umap(combat_corrected_fecha, n_neighbors = 10, metric = "euclidean", min_dist = 0.3)
...
# Graficar UMAP con ggplot2
library(ggplot2)
UMAP_postCombatfecha_equipo<-ggplot(umap_coor_combat, aes(x = UMAP1, y = UMAP2, color = equipo, shape = covid)) +
  geom_point(size = 3) +
  labs(title = "UMAP equipo-Post Combat fecha",
       x = "UMAP1",
       y = "UMAP2") +
  theme_minimal()


UMAP_postCombatfecha_fecha<-ggplot(umap_coor_combat, aes(x = UMAP1, y = UMAP2, color = fecha, shape = covid)) +
  geom_point(size = 3) +
  labs(title = "UMAP fecha-Post Combat fecha",
       x = "UMAP1",
       y = "UMAP2") +
  theme_minimal()
```
...
A <- UMAP_postCombatfecha_equipo +
  coord_equal() +
  labs(color = "equipo", shape = "covid")

B <- UMAP_postCombatfecha_fecha +
  coord_equal() +
  labs(color = "fecha", shape = "covid")
...
print(combined_plot_UMAPpost_combatfecha_combat_todos)
```

```{r}
save(combined_plot_UMAPpost_combatfecha_combat_todos, file = "combined_plot_UMAPpost_combatfecha_combat_todos.rda")
```
...
# pca_kruskal_COMBATfecha_todos<- ggplot(factor_summary, aes(x = reorder(Factor, MeanLogPValue), y = MeanLogPValue, fill = Factor)) +
#   geom_bar(stat = "identity") +
#   coord_flip() +
#   theme_minimal() +
#   labs(title = "Importancia de los Factores -PCA",
...
pca_kruskal_COMBATfecha_todos <- ggplot(factor_summary_umap,
  aes(x = reorder(Factor, MeanLogPValue), y = MeanLogPValue, fill = Factor)) +
  geom_col() +
  coord_flip() +
  theme_minimal() +
  labs(title = "Importancia de factores PCA",
       x = "Factor", y = "-log10(P-valor medio)") +
  theme(legend.position = "none")
print(pca_kruskal_COMBATfecha_todos)
```
...
# 4) Gráfico (sin '+' colgando)
umap_postcombatfecha_var_todos <- ggplot(factor_summary_umap,
                                    aes(x = reorder(Factor, MeanLogPValue),
                                        y = MeanLogPValue, fill = Factor)) +
  geom_col() +
  coord_flip() +
...
print(umap_postcombatfecha_var_todos)
...
combined_var_combat_fecha <- (
  pca_kruskal_COMBATfecha_todos + umap_postcombatfecha_var_todos
)

# Agregamos los títulos y anotaciones
combined_var_combat_fecha <- combined_var_combat_fecha + 
  plot_annotation(
    title = "Influencia de los factores en los datos post Combat por fecha"
    #tag_levels = "A"  # Etiquetas A, B, C, ...
  ) + 
  plot_layout(guides = "collect", widths = c(1, 1))  # Para asegurar que ambos gráficos tengan el mismo ancho

# Para ver el gráfico
print(combined_var_combat_fecha)


```

```{r}
save(combined_var_combat_fecha, file = "combined_var_combat_fecha.rda")
```
...
# Dibuja el dendrograma
plot(dend, main = "Dendrograma COVID-Post Combat equipo")
legend("topright",           # Posición de la leyenda
       legend = c("COVID Negativo", "COVID Positivo"), # Textos de la leyenda
       fill = c("red", "blue"),                        # Colores de la leyenda
       border = "black",                               # Borde de las cajas de color
...
grid.echo()
g <- grid.grab()
cluster_combatxfecha_todos <- ggplotify::as.ggplot(g)  # <- ahora es un ggplot
```

```{r}
save(cluster_combatxfecha_todos, file= "cluster_combatxfecha_todos.rda")
```
...
# Verificar que los datos estén filtrados correctamente
table(Datos_actualizados$fecha)
...
# Crear un vector de colores para las etiquetas basado en 'equipo'
fecha_factor <- as.factor(Datos_actualizados$fecha)
pal <- rainbow(length(levels(fecha_factor))) # Paleta de colores
label_colors <- palette[fecha_factor]           # Asignar colores por equipo
...
# Dibujás en base R
plot(dend_fecha, main = "Clustering Diferenciado por Fecha post ComBat-Fecha")
legend("topright", legend = levels(fecha_factor), col = pal,  title = "Fecha",   
  pch = 19,
  cex = 0.7,
  bty = "n"
)
...
grid.echo()
g <- grid.grab()
cluster_fechacombatxfecha_todos <- ggplotify::as.ggplot(g)  # <- ahora es un ggplot
```

```{r}
save(cluster_fechacombatxfecha_todos, file="cluster_fechacombatxfecha_todos.rda")
```
...
# Dibujás en base R
plot(dend_fecha, main = "Clustering Diferenciado por equipo post ComBat-Fecha")
legend("topright", legend = levels(equipo_factor), col = pal,  title = "Fecha",   
  pch = 19,
  cex = 0.7,
  bty = "n"
)
...
grid.echo()
g <- grid.grab()
cluster_equipocombatxfecha_todos <- ggplotify::as.ggplot(g)  # <- ahora es un ggplot
```

```{r}
save(cluster_equipocombatxfecha_todos, file="cluster_equipocombatxfecha_todos.rda")
```
...
-----------------------------------------------------------------------------------
```{r}
plot_dendrogram <- function(matriz, metadata, filtro = NULL, variable_color,
                            titulo = "", nombre_archivo = NULL, palette_name = "Paired") {
  library(dendextend)
  library(RColorBrewer)

  # Aplicar filtro si se indica (por ejemplo: solo positivos o negativos)
  if (!is.null(filtro)) {
    matriz <- matriz[filtro, ]
    metadata <- metadata[filtro, ]
  }

  # Factor para colorear etiquetas
  color_factor <- as.factor(metadata[[variable_color]])
  num_colores <- length(levels(color_factor))

  # Crear paleta de colores
  palette <- if (num_colores <= 12) {
    brewer.pal(num_colores, palette_name)
  } else {
    colorRampPalette(brewer.pal(12, palette_name))(num_colores)
  }
  label_colors <- palette[color_factor]

  # Clustering
  dist_mat <- dist(matriz)
  ...
  # Si hay nombre de archivo, guardar como PNG
  if (!is.null(nombre_archivo)) {
    png(nombre_archivo, width = 800, height = 600)
    plot(dend, main = titulo)
    legend("topright", legend = levels(color_factor), col = palette, pch = 19,
           title = variable_color, cex = 0.7, bty = "n")
    dev.off()
  }

  # Mostrar en visor de RStudio
  plot(dend, main = titulo)
  legend("topright", legend = levels(color_factor), col = palette, pch = 19,
         title = variable_color, cex = 0.7, bty = "n")
}
...
plot_dendrogram <- function(matriz, metadata, variable_color,
                            titulo = "Dendrograma", nombre_archivo = NULL,
                            devolver = c("none","recorded","ggplot")) {
  devolver <- match.arg(devolver)

  ## ... (tu código que dibuja el dendrograma y la leyenda) ...
  ## plot(...); legend(...)

  # si querés guardar a archivo dentro de la función
  if (!is.null(nombre_archivo)) {
    png(nombre_archivo, width = 1200, height = 800, res = 150)
    ## repetir las dos líneas de dibujo o encapsularlas en una función interna
    ## plot(...); legend(...)
    dev.off()
  }

  if (devolver == "recorded") {
    return(grDevices::recordPlot())
  } else if (devolver == "ggplot") {
    grid.echo(); g <- grid.grab()
...
```{r}
# 1. Clustering de POSITIVOS por FECHA
# plot_dendrogram(
#   matriz = combat_corrected_fecha,
#   metadata = Datos_actualizados,
#   filtro = Datos_actualizados$PCR.Cov == "Cov.Pos",
#   variable_color = "fecha",
#   titulo = "Clustering de Positivos por Fecha-Combat Fecha",
#   nombre_archivo = "POS_combat_fecha.png"
# )
# 
# # 2. Clustering de NEGATIVOS por FECHA
# plot_dendrogram(
#   matriz = combat_corrected_fecha,
#   metadata = Datos_actualizados,
#   filtro = Datos_actualizados$PCR.Cov == "Cov.Neg",
#   variable_color = "fecha",
#   titulo = "Clustering de Negativos por Fecha-Combat Fecha",
#   nombre_archivo = "NEG_combat_fecha.png"
# )

# 3. Clustering general por EQUIPO
plot_dendrogram(
  matriz = combat_corrected_fecha,
  metadata = Datos_actualizados,
  variable_color = "equipo",
  titulo = "Clustering por Equipo-Combat Fecha",
  nombre_archivo = "EQUIPO_combat.png"
)


# 4. Clustering general por FECHA (sin filtrar)
plot_dendrogram(
  matriz = combat_corrected_fecha,
  metadata = Datos_actualizados,
  variable_color = "fecha",
  titulo = "Clustering por Fecha-Combat Fecha",
  nombre_archivo = "FECHA_combat.png"
)
...
```{r}
combat_corrected_fecha_tot <- combat_corrected_fecha
```

```{r}
combat_corrected_equipo_tot <- as.data.frame(combat_corrected_equipo)
