...
```{r}
featureMatrix_label<-featureMatrix[, c(1:60)]
```

```{r}
featureMatrix_bind<- cbind(featureMatrix_label, Y=Datos_actualizados$PCR.Cov)#1=neg,2=pos
```
...
split_data <- initial_split(featureMatrix_bind_nodic_carga, strata = "Y", prop = 0.8)
...
```{r}
# ciegoMatrix_nodic_carga <- ciegoMatrix_nodic_carga %>%
#   rename(Y = label)
```

```{r}
ciego_data <- as.data.frame(ciegoMatrix_nodic_carga)
...
# Crear un dataframe con todas las métricas
metricsRF_matrix_carga_ciego <- data.frame(
  Metric = c("Accuracy", "Kappa", "Sensitivity", "Specificity", "Precision", 
             "F1_Score", "NPV", "Prevalence", "Detection_Rate", 
             "Balanced_Accuracy", "LR+", "LR-", "AUC"),
...
# Mostrar los resultados
print(metricsRF_matrix_carga_ciego)
...
split_data <- initial_split(featureMatrix_bind_dic_carga, strata = "Y", prop = 0.8)
