...
prediccionesRF_carga<-predict(RF, newdata = testData_NOID)#matriz binarizada
...
```{r}
#prediccionesRF1<-predict(resultadosRF, newdata = dataNOID_test)
predGLM_matrix_carga<-predict(GLM, newdata = testData_NOID)#matriz binarizada
print(predGLM)
...
```{r}
#prediccionesRF1<-predict(resultadosRF, newdata = dataNOID_test)
prediccionesKNN_matrix_carga<-predict(KNN, newdata = testData_NOID)#matriz binarizada
...
```{r}
library(caret)
accuracyKNN_matrix_carga <- RF_metrics_equipo$overall["Accuracy"]
kappaKNN_matrix_carga <- RF_metrics_equipo$overall["Kappa"]
# Métricas por clase
sensitivityKNN_matrix_carga <- RF_metrics_equipo$byClass["Sensitivity"]
specificityKNN_matrix_carga <- RF_metrics_equipo$byClass["Specificity"]
precisionKNN_matrix_carga <- RF_metrics_equipo$byClass["Pos Pred Value"]
recallKNN_matrix_carga <- RF_metrics_equipo$byClass["Sensitivity"]  # Igual a sensitivity
f1_scoreKNN_matrix_carga <- RF_metrics_equipo$byClass["F1"]
npvKNN_matrix_carga <- RF_metrics_equipo$byClass["Neg Pred Value"]
prevalenceKNN_matrix_carga <- RF_metrics_equipo$byClass["Prevalence"]
detection_rateKNN_matrix_carga <- RF_metrics_equipo$byClass["Detection Rate"]
balanced_accuracyKNN_matrix_carga <- RF_metrics_equipo$byClass["Balanced Accuracy"]
...
thr_filtered <- thr[colnames(trainData_matriz_dico[,1:38])]
```
...
```{r}
test_subset_matriz <- test_subset_matriz[, colnames(trainData_matriz_dico)[1:38]]
...
library(dplyr)
testData_matriz_dico<-as.data.frame(testData_matriz_dico)
testData_matriz_dico <- testData_matriz_dico %>% rename(Y = V39)
str(testData_matriz_dico$Y)
```
