...
split_data <- initial_split(combat_corrected_equipo_covid, strata = "Y", prop = 0.8)
...
split_data <- initial_split(combat_corrected_equipo_covid, strata = "label", prop = 0.8)
...
```{r}
trainData_equipo_dico<-trainData_equipo_dico[1:62]
```


```{r}
train_subset <- trainData_equipo_dico[, 1:61]
```
...
library(dplyr)
trainData_equipo_dico<-as.data.frame(trainData_equipo_dico)
trainData_equipo_dico <- trainData_equipo_dico %>% rename(Y = V62)
str(trainData_equipo_dico$Y)
```
...
```{r}
zero_var_indices <- nearZeroVar(trainData_equipo_dico[,1:61])
...
```{r}
test_subset <- testData_equipo_dico[, 1:61]
```
...
```{r}
test_subset <- test_subset[, colnames(trainData_equipo_dico)[1:37]]
#test_subset <-  test_subset[, colnames(trainData_equipo_dico)]
thr_filtered <-  thr[colnames(trainData_equipo_dico)[1:37]]
#Dicotimizacion de la matriz de intensidad
...
library(dplyr)
testData_equipo_dico<-as.data.frame(testData_equipo_dico)
testData_equipo_dico <- testData_equipo_dico %>% rename(Y = V38)
str(testData_equipo_dico$Y)
```
