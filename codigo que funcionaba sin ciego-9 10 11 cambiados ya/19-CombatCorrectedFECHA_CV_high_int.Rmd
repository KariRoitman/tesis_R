...
thr_filtered <- thr[colnames(trainData_fecha_dico[,1:39])]
...
```{r}
test_subset_fecha <- test_subset_fecha[, colnames(trainData_fecha_dico[,1:39])]
...
library(dplyr)
testData_fecha_dico<-as.data.frame(testData_fecha_dico)
testData_fecha_dico <- testData_fecha_dico %>% rename(Y = V40)
str(testData_fecha_dico$Y)
```
