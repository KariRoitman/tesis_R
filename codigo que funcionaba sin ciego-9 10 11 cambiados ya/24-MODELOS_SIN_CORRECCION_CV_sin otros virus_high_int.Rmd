...
Test.rf_matrix$Y <- factor(Test.rf_matrix$Y, levels = c(1, 2), labels = c("X1", "X2"))
...
y_test <- factor(y_test, levels = c("X1", "X2"), labels = c("Cov.Neg", "Cov.Pos"))
```
...
library(caret)
accuracyRANGER_matrix_cc <- RF_metrics_equipo$overall["Accuracy"]
kappaRANGER_matrix_cc <- RF_metrics_equipo$overall["Kappa"]
# Métricas por clase
sensitivityRANGER_matrix_cc <- RF_metrics_equipo$byClass["Sensitivity"]
specificityRANGER_matrix_cc <- RF_metrics_equipo$byClass["Specificity"]
precisionRANGER_matrix_cc <- RF_metrics_equipo$byClass["Pos Pred Value"]
recallRANGER_matrix_cc <- RF_metrics_equipo$byClass["Sensitivity"]  # Igual a sensitivity
f1_scoreRANGER_matrix_cc <- RF_metrics_equipo$byClass["F1"]
npvRANGER_matrix_cc <- RF_metrics_equipo$byClass["Neg Pred Value"]
prevalenceRANGER_matrix_cc <- RF_metrics_equipo$byClass["Prevalence"]
detection_rateRANGER_matrix_cc <- RF_metrics_equipo$byClass["Detection Rate"]
balanced_accuracyRANGER_matrix_cc <- RF_metrics_equipo$byClass["Balanced Accuracy"]
...
```{r}
test_subset_matriz <- testData_matriz_dico[, 1:61]
```
