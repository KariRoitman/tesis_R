...
```{r}
featureMatrix_label<-featureMatrix[, c(2:62,64)]
```
...
prediccionesRF_fecha_covid<-predict(RF, newdata = testData_NOID)#matriz binarizada
...
```{r}
train_subset_matriz <- trainData_matriz_dico[, 1:61]
```
...
```{r}
library(dplyr)
trainData_matriz_dico <- trainData_matriz_dico %>% rename(Y = V62)
...
```{r}
zero_var_indices <- nearZeroVar(trainData_matriz_dico[,1:61])
...
test_subset_matriz <- testData_matriz_dico[, colnames(trainData_matriz_dico)[1:41]]
```
...
library(dplyr)
testData_matriz_dico<-as.data.frame((testData_matriz_dico))
str(testData_matriz_dico$V42)
colnames(testData_matriz_dico)[ colnames(testData_matriz_dico) == "V42" ] <- "Y"
