...
split_data <- initial_split(combat_corrected_fecha_covid, strata = "label", prop = 0.8)
...
thr_filtered <- thr[colnames(trainData_fecha_dico[,1:37])]
...
```{r}
test_subset_fecha <- test_subset_fecha[, colnames(trainData_fecha_dico[,1:37])]
...
library(dplyr)
testData_fecha_dico<-as.data.frame(testData_fecha_dico)
testData_fecha_dico <- testData_fecha_dico %>% rename(Y = V38)
str(testData_fecha_dico$Y)
```
...
split_data <- initial_split(combat_corrected_fecha_covid, strata = "Y", prop = 0.8)
