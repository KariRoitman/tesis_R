...
# Crear un dataframe con todas las métricas
metricsKNN_equipo_dic <- data.frame(
  Metric = c("Accuracy", "Kappa", "Sensitivity", "Specificity", "Precision", 
             "F1_Score", "NPV", "Prevalence", "Detection_Rate", 
             "Balanced_Accuracy", "LR+", "LR-", "AUC"),
...
```{r}
library(caret)
accuracyRanger_equipo_dich_carga <- RF_metrics_equipo$overall["Accuracy"]
kappaRanger_equipo_dich_carga <- RF_metrics_equipo$overall["Kappa"]
# Métricas por clase
sensitivityRanger_equipo_dich_carga <- RF_metrics_equipo$byClass["Sensitivity"]
specificityRanger_equipo_dich_carga <- RF_metrics_equipo$byClass["Specificity"]
precisionRanger_equipo_dich_carga <- RF_metrics_equipo$byClass["Pos Pred Value"]
recallRanger_equipo_dich_carga <- RF_metrics_equipo$byClass["Sensitivity"]  # Igual a sensitivity
f1_scoreRanger_equipo_dich_carga <- RF_metrics_equipo$byClass["F1"]
npvRanger_equipo_dich_carga <- RF_metrics_equipo$byClass["Neg Pred Value"]
prevalenceRanger_equipo_dich_carga <- RF_metrics_equipo$byClass["Prevalence"]
detection_rateRanger_equipo_dich_carga <- RF_metrics_equipo$byClass["Detection Rate"]
balanced_accuracyRanger_equipo_dich_carga <- RF_metrics_equipo$byClass["Balanced Accuracy"]
