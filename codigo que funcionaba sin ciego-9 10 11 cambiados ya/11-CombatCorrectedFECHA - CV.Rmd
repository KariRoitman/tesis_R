...
thr_filtered <- thr[colnames(trainData_fecha_dico[,1:41])]
...
```{r}
test_subset_fecha <- test_subset_fecha[, colnames(trainData_fecha_dico[,1:41])]
...
library(dplyr)
testData_fecha_dico<-as.data.frame(testData_fecha_dico)
testData_fecha_dico <- testData_fecha_dico %>% rename(Y = V42)
str(testData_fecha_dico$Y)
```
