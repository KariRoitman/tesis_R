...
split_data <- initial_split(featureMatrix_bind_nodic_tot, strata = "Y", prop = 0.8)
...
```{r}
ciegoMatrix_nodico_tot <- as.data.frame(ciegoMatrix_nodico_tot)

# Si viene con columna Y (conocida), separala:
if ("Y" %in% colnames(ciegoMatrix_nodico_tot)) {
  Y_ciego <- ciegoMatrix_nodico_tot$Y
  X_ciego <- ciegoMatrix_nodico_tot[, setdiff(colnames(ciegoMatrix_nodico_tot), "Y")]
} else {
  X_ciego <- ciegoMatrix_nodico_tot
  Y_ciego <- NULL
}
...
```{r}
# Predicciones (clase)
predicciones_ciego_matriz_tot <- predict(RF, newdata = X_ciego)
...
#prob_ciego$Cov.Pos<-prob_ciego$X2
  ...
  # Confusion matrix
  ciego_tot_equipo<-caret::confusionMatrix(predicciones_ciego_matriz_tot, Y_ciego, positive = "Cov.Pos")
...
split_data <- initial_split(featureMatrix_bind_dic_tot, strata = "Y", prop = 0.8)
