...
rf_cv<-function(df_train, model, grid, metrica, title){
...
res_1<-rf_cv(df_train=Train.rf_equipo, model="ranger", grid=data.frame(mtry = 2, min.node.size = 1, splitrule = "gini"), metrica="Accuracy", title = "Primera_funcion" )
res_1<-print(ranger_equipo)

save(res_1, file = paste(title, ".rda"))
}
...
prob_ciego <- prob_ciego %>%
  ...
  # Confusion matrix
  ciego_tot_equipo<-caret::confusionMatrix(predicciones_ciego, Y_ciego, positive = "Cov.Pos")
