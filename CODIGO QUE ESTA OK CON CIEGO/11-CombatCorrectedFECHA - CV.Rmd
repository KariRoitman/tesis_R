...
split_data <- initial_split(combat_corrected_fecha_tot_dic, strata = "label", prop = 0.8)
...
```{r}
zero_var_indices <- nearZeroVar(trainData_fecha_dico[,-ncol(train_subset_fecha)])
...
GLM_metrics_equipo <- caret::confusionMatrix(predGLM_fecha_dic, y_test, positive = "Cov.Pos")
```
...
# Carga los paquetes necesarios
library(caret)
accuracyGLM_fecha_dic <- GLM_metrics_equipo$overall["Accuracy"]
kappaGLM_fecha_dic <- GLM_metrics_equipo$overall["Kappa"]
# Métricas por clase
sensitivityGLM_fecha_dic <- GLM_metrics_equipo$byClass["Sensitivity"]
specificityGLM_fecha_dic <- GLM_metrics_equipo$byClass["Specificity"]
precisionGLM_fecha_dic <- GLM_metrics_equipo$byClass["Pos Pred Value"]
recallGLM_fecha_dic <- GLM_metrics_equipo$byClass["Sensitivity"]  # Igual a sensitivity
f1_scoreGLM_fecha_dic <- GLM_metrics_equipo$byClass["F1"]
npvGLM_fecha_dic <- GLM_metrics_equipo$byClass["Neg Pred Value"]
prevalenceGLM_fecha_dic <- GLM_metrics_equipo$byClass["Prevalence"]
detection_rateGLM_fecha_dic <- GLM_metrics_equipo$byClass["Detection Rate"]
balanced_accuracyGLM_fecha_dic <- GLM_metrics_equipo$byClass["Balanced Accuracy"]
...
```{r}
ciego_combat_corrected_fecha_tot_nodic <- as.data.frame(ciego_combat_corrected_fecha_tot_nodic)
...
```{r}
# Predicciones (clase)
predicciones_ciego_fecha_tot <- predict(RF_fecha, newdata = X_ciego)
levels(predicciones_ciego_fecha_tot) <- c("Cov.Neg", "Cov.Pos")

# Predicciones (probabilidades)
prob_ciego <- predict(RF_fecha, newdata = X_ciego, type = "prob")
