...
```{r}
combat_corrected_equipo_covid<- cbind(combat_corrected_equipo_covid, label=Datos_actualizados$PCR.Cov, id=Datos_actualizados$spot.a.1)#1=neg,2=pos
```

```{r}
combat_corrected_equipo_covid<-as.data.frame(combat_corrected_equipo_covid)
combat_corrected_equipo_covid <- combat_corrected_equipo_covid %>%
  mutate(across(1:73, as.numeric))
combat_corrected_equipo_covid$label<-as.factor(combat_corrected_equipo_covid$label)
str(combat_corrected_equipo_covid)
```
...
```{r}
library(dplyr)
combat_corrected_equipo_covid <- combat_corrected_equipo_covid %>% rename(Y = label)
str(combat_corrected_equipo_covid$Y)
```



```{r}
combat_corrected_equipo_solocovid<-combat_corrected_equipo_covid[1:490,1:74]

ciego_combat_corrected_equipo_covid<-combat_corrected_equipo_covid[491:671,1:74]
```
...
# Métricas generales
accuracy_rf_covid <- RF_metrics_equipo$overall["Accuracy"]
kappa_rf_covid <- RF_metrics_equipo$overall["Kappa"]
...
```{r}
combat_corrected_equipo_solocovid_dic<-combat_corrected_equipo_tot[1:490,]

ciego_combat_corrected_equipo_solocovid_dic<-combat_corrected_equipo_tot[491:671,]
```
...
split_data <- initial_split(combat_corrected_equipo_solocovid_dic, strata = "label", prop = 0.8)
...
```{r}
trainData_equipo_dico<-as.data.frame(trainData_equipo_dico[,-ncol(trainData_equipo_dico)])
```
