...
for (i in 1:nrow(hyper_grid)) {
  ...
  predictions <- as.data.frame(rangerHC$predictions)
...
print(hyper_grid$AUC[i] <- auc_rangerHC)
...
```{r}
SVM_HC <- train(Y ~., data = trainData, method = "svmRadial", trControl = control1, preProcess = c("center","scale"), tuneLength = 10, prob.model= TRUE)
# Print the best tuning parameter sigma and C that maximizes model accuracy
SVM_HC$bestTun    #there is a tuning parameter C, also known as Cost, that determines the possible misclassifications. It essentially imposes a penalty to the model for making an error: the higher the value of C, the less likely it is that the SVM algorithm will misclassify a point.
...
best_model_index <- apply(GLM_HC$results, 1, function(row) {
  all(row[1:length(best_params)] == as.numeric(best_params))
})

best_model_results <- GLM_HC$results[best_model_index, ]
...
prediccionesRF<-predict(RF_HC, newdata = testData_NOID)#matriz binarizada
...
```{r}
# Precisión (<)
accuracy_RF_HC <- sum(diag(confusion_matrixRF)) / sum(confusion_matrixRF)
print(paste("La precisión (Accuracy) es:", round(accuracy_RF_HC,2)))

# Sensibilidad (Recall o TPR)
sensitivity_RF_HC <- confusion_matrixRF[2, 2] / sum(confusion_matrixRF[, 2])
print(paste("La sensibilidad es:", round(sensitivity_RF_HC,2)))

# Especificidad (TNR)
specificity_RF_HC <- confusion_matrixRF[1, 1] / sum(confusion_matrixRF[, 1])
print(paste("la especificidad es:", round(specificity_RF_HC,2)))

# Valor Predictivo Positivo (PPV o Precision)
ppv_RF_HC <- confusion_matrixRF[2, 2] / sum(confusion_matrixRF[2, ])
print(paste("el VPP es:", round(ppv_RF_HC,2)))

# Valor Predictivo Negativo (VPN)
npv_RF_HC <- confusion_matrixRF[1, 1] / sum(confusion_matrixRF[1, ])
print(paste("el VPN es: ",  round(npv_RF_HC,2)))
...
prob_predRF <- predict(RF_HC, newdata = testData_NOID, type = "prob")
...
AUC_RF_HC <- auc(roc_curve)

print(paste("El valor de AUC es:", round(AUC_RF_HC, 3)))
...
```{r}
#prediccionesRF1<-predict(resultadosRF, newdata = dataNOID_test)
prediccionesSVM<-predict(SVM_HC, newdata = testData_NOID)#matriz binarizada
...
#metricas
# Precisión
accuracy_SVM_HC <- sum(diag(confusion_matrixSVM)) / sum(confusion_matrixSVM)
print(accuracy_SVM_HC)
# Precisión positiva (VP) o sensibilidad
sensitivity_SVM_HC <- confusion_matrixSVM[2, 2] / sum(confusion_matrixSVM[,2 ])
print(sensitivity_SVM_HC)
# Precisión negativa (VN) o especificidad
specificity_SVM_HC <- confusion_matrixSVM[1, 1] / sum(confusion_matrixSVM[,1 ])
specificity_SVM_HC

# Valor predictivo positivo (VPP)
ppv_SVM_HC <- confusion_matrixSVM[2, 2] / sum(confusion_matrixSVM[2, ])
print(ppv_SVM_HC)
# Valor predictivo negativo (VPN)
npv_SVM_HC <- confusion_matrixSVM[1, 1] / sum(confusion_matrixSVM[1, ])
print(npv_SVM_HC)




prob_predSVM_HC <- predict(SVM_HC, newdata = testData_NOID, type = "prob")


# Verifica los nombres de las columnas
print(colnames(prob_predSVM_HC))  # Debería mostrar "1" y "2"
...
roc_curve <- roc(response = y_test_numeric, predictor = prob_predSVM_HC[, "2"], levels = c(1, 2))
AUC_SVM_HC <- auc(roc_curve)

print(paste("El valor de AUC es:", round(AUC_SVM_HC, 3)))
...
```{r}
#prediccionesRF1<-predict(resultadosRF, newdata = dataNOID_test)
prediccionesKNN<-predict(KNN_HC, newdata = testData_NOID)#matriz binarizada
...
#metricas
# Precisión
accuracy_KNN_HC <- sum(diag(confusion_matrixKNN)) / sum(confusion_matrixKNN)
print(accuracy_KNN_HC)
# Precisión positiva (VP) o sensibilidad
sensitivity_KNN_HC <- confusion_matrixKNN[2, 2] / sum(confusion_matrixKNN[,2 ])
print(sensitivity_KNN_HC)
# Precisión negativa (VN) o especificidad
specificity_KNN_HC <- confusion_matrixKNN[1, 1] / sum(confusion_matrixKNN[,1 ])
specificity_KNN_HC

# Valor predictivo positivo (VPP)
ppv_KNN_HC <- confusion_matrixKNN[2, 2] / sum(confusion_matrixKNN[2, ])
print(ppv_KNN_HC)
# Valor predictivo negativo (VPN)
npv_KNN_HC <- confusion_matrixKNN[1, 1] / sum(confusion_matrixKNN[1, ])
print(npv_KNN_HC)


prob_predKNN <- predict(KNN_HC, newdata = testData_NOID, type = "prob")
...
# Calcula el AUC
AUC_KNN_HC <- auc(roc_curve_knn)
print(paste("El valor de AUC es:", round(AUC_KNN_HC, 3)))
...
```{r}
#prediccionesRF1<-predict(resultadosRF, newdata = dataNOID_test)
predGLM_HC<-predict(GLM_HC, newdata = testData_NOID)#matriz binarizada
print(predGLM_HC)
...
```{r}
confusion_matrixGLM <- table(predGLM_HC, y_test)
# Imprimir la matriz de confusión
...
```{r}
# Precisión (Accuracy)
accuracy_GLM_HC <- sum(diag(confusion_matrixGLM)) / sum(confusion_matrixGLM)
print(paste("La precisión (Accuracy) es:", round(accuracy_GLM_HC,2)))

# Sensibilidad (Recall o TPR)
sensitivity_GLM_HC <- confusion_matrixGLM[2, 2] / sum(confusion_matrixGLM[, 2])
print(paste("La sensibilidad es:", round(sensitivity_GLM_HC,2)))

# Especificidad (TNR)
specificity_GLM_HC <- confusion_matrixGLM[1, 1] / sum(confusion_matrixGLM[, 1])
print(paste("la especificidad es:", round(specificity_GLM_HC,2)))

# Valor Predictivo Positivo (PPV o Precision)
ppv_GLM_HC<- confusion_matrixGLM[2, 2] / sum(confusion_matrixGLM[2, ])
print(paste("el VPP es:", round(ppv_GLM_HC,2)))

# Valor Predictivo Negativo (VPN)
npv_GLM_HC <- confusion_matrixGLM[1, 1] / sum(confusion_matrixGLM[1, ])
print(paste("el VPN es: ",  round(npv_GLM_HC,2)))




# Predicciones probabilísticas con el modelo GLM
prob_predGLM_HC<- predict(GLM_HC, newdata = testData_NOID, type = "prob")

# Verifica los nombres de las columnas (clases)
print(colnames(prob_predGLM_HC))  # Deberían ser "1" y "2" o los nombres de las clases

# Asegúrate de que y_test sea numérico con niveles 1 y 2
y_test_numeric <- as.numeric(testData$Y)

# Calcula la curva ROC usando la probabilidad de la clase positiva ("2")
roc_curve_glm <- roc(response = y_test_numeric, predictor = prob_predGLM_HC[, "2"], levels = c(1, 2))

# Calcula el AUC
AUC_GLM_HC <- auc(roc_curve_glm)
print(paste("El valor de AUC es:", round(AUC_GLM_HC, 3)))
...
```{r}
resultados_modelosHC <- data.frame(
  Modelo = c("RF_model", "GLM_model", "SVM_model", "KNNmodel", "Rangermodel"),
  Accuracy = c(accuracy_RF_HC, accuracy_GLM_HC , accuracy_SVM_HC, accuracy_KNN_HC, best_rangerHC$Accuracy),
  Sensibilidad=c(sensitivity_RF_HC, sensitivity_GLM_HC, sensitivity_SVM_HC, sensitivity_KNN_HC, best_rangerHC$Sensitivity),
  Especificidad = c(specificity_RF_HC, specificity_GLM_HC, specificity_SVM_HC, specificity_KNN_HC, best_rangerHC$Specificity ),
  AUC= c(AUC_RF_HC, AUC_GLM_HC, AUC_SVM_HC, AUC_KNN_HC,  best_rangerHC$AUC)
)

print(resultados_modelosHC)
