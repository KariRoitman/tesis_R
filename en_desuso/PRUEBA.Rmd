...
```{r}
class(INBIRS.1.df.f.1)
```


```{r}
isS4(Espectros.INBIRS.1[1])
```

```{r}
Espectros.INBIRS.1[1:3
                   ]
```
...
espectros_df <- data.frame(
  ID = names(Espectros.INBIRS.1),
  NumMZValues = sapply(Espectros.INBIRS.1, function(x) x@Number_of_m.z_values),
  RangeMZ = sapply(Espectros.INBIRS.1, function(x) x@Range_of_m.z_values),
  RangeIntensity = sapply(Espectros.INBIRS.1, function(x) x@Range_of_intensity_values),
  stringsAsFactors = FALSE
)

# Ahora intentamos el join
resultado <- merge(espectros_df, INBIRS.1.df.f.1, by.x = "ID", by.y = 1)
...
plot(Espectros.INBIRS.1[[1]], main = "Espectros Superpuestos", col = "blue", type = "l")

# Iterar sobre los espectros restantes y agregarlos al gráfico
for (i in 2:length(Espectros.INBIRS.1)) {
  lines(Espectros.INBIRS.1[[i]], col = i)  # Añadir líneas para cada espectro, con diferentes colores
}

# Añadir una leyenda para indicar los colores de cada espectro
legend("topright", legend = 1:length(Espectros.INBIRS.1), col = 1:length(Espectros.INBIRS.1),
...
Equipo (HdC+INBIRS es HdC, Malbrán y CR)
...
Explorar Ct data (regresión por Random Forest, separar datos en funcion de valores de Ct)

Correr Rf y SVM con un tunning apropiado. Probar primero modelos selectivos para cada equipo (HdC+INBIRS es HdC, Malbrán y CR), luego intentar un modelo que combine todos los datos.
