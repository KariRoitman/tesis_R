...
```{r}
# # Calcular la cantidad de niveles en "fecha"
# num_colores <- length(unique(umap_df$fecha))
# 
# # Generar una paleta automática (con RColorBrewer)
...
```{r}
ggsave(
  filename = "combined_plot_UMAP_covid.png",
...
combined_plot_tSNE_covid <- (g1 | g2) +
  plot_annotation(
...
# Mostrar
print(combined_plot_tSNE_covid)
...
```{r}
ggsave(
  filename = "combined_plot_tSNE_covid.png",
  plot = combined_plot_tSNE_covid,
  width = 12,
  height = 10,
  dpi = 300
)
...
equipo_factor <- factor(Datos_actualizados_cov_pos$equipo, levels = c("CR", "HC", "Malbran"))
...
plot_dendrogram(featureMatrix_num, COVID,
                ...
                nombre_png = "dendrograma_soloCOVID.png")
...
# Crear un vector de colores para las etiquetas basado en 'fecha'
fecha_factor <- as.factor(Datos_actualizados_cov_neg$fecha)
palette <- rainbow(length(levels(fecha_factor))) # Paleta de colores
label_colors <- palette[fecha_factor]
...
```
```{r}
combat_corrected_fecha_covid <- combat_corrected_fecha
```


```{r}
combat_corrected_equipo_covid<-combat_corrected_equipo
```
