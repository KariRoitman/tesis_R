...
RF<-RF_matrix_nodic_cargafx(df_train=trainData, model="rf", grid=hiperparametros, metrica="Accuracy", control=cross_val_rf )
print(RF)
```
...
```{r}
# Calcular CV en picos dicotomizados (excluyendo la columna Y)
cv_picos_dico <- apply(trainData_matriz_dico[, -ncol(trainData_matriz_dico)], 2, function(col) {
  media <- mean(col)
  sd <- sd(col)
  if (media == 0) return(Inf) else return((sd / media) * 100)
})

# Filtrar nombres de picos con CV < 40
picos_cv_estables <- names(cv_picos_dico[cv_picos_dico < 40])

# Filtrar sets de entrenamiento y testeo con los mismos picos
trainData_matriz_dico_filtrado <- trainData_matriz_dico[, c(picos_cv_estables, "Y")]
testData_matriz_dico_filtrado <- testData_matriz_dico[, c(picos_cv_estables, "Y")]
trainData_matriz_dico<-trainData_matriz_dico_filtrado
testData_matriz_dico<-testData_matriz_dico_filtrado
```
...
# Crear un dataframe con todas las métricas
metricsRanger_matrix_dich_cc_ciego_fx <- data.frame(
  Metric = c("Accuracy", "Kappa", "Sensitivity", "Specificity", "Precision", 
             "F1_Score", "NPV", "Prevalence", "Detection_Rate", 
             "Balanced_Accuracy", "LR+", "LR-", "AUC"),
...
# Mostrar los resultados
print(metricsRanger_matrix_dich_cc_ciego_fx)
```
