...
thr_filtered <- thr[colnames(trainData_matriz_dico[,1:47])]
```
...
```{r}
test_subset_matriz <- test_subset_matriz[, colnames(trainData_matriz_dico)[1:47]]
...
library(dplyr)
testData_matriz_dico<-as.data.frame(testData_matriz_dico)
testData_matriz_dico <- testData_matriz_dico %>% rename(Y = V48)
str(testData_matriz_dico$Y)
```
...
```{r}
predRF_matriz_dico<-caret::confusionMatrix(data =  prediccionesRFd,       
                      reference =  factor(y_test), positive = "X2")

predRF_matriz_dico$overall
predRF_matriz_dico$byClass
```
