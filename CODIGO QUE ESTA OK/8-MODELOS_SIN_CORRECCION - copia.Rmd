...
roc_curve_glm <- roc(response = y_test_numeric, predictor = prob_predGLM[, "Cov.Pos"], levels = c(1, 2))
...
# Calcula la curva ROC usando la probabilidad de la clase positiva ("2")
roc_curve_knn <- roc(response = y_test_numeric, predictor = prob_predKNN[, "Cov.Pos"], levels = c(1, 2))
...
y_test <- as.numeric(factor(Test.rf_matrix$Y, levels = c("Cov.Neg", "Cov.Pos"), labels = c(1, 2)))
...
thr_filtered <- thr[colnames(trainData_matriz_dico[,1:48])]
```
...
```{r}
test_subset_matriz <- test_subset_matriz[, colnames(trainData_matriz_dico)[1:48]]
...
library(dplyr)
testData_matriz_dico<-as.data.frame(testData_matriz_dico)
testData_matriz_dico <- testData_matriz_dico %>% rename(Y = V49)
str(testData_matriz_dico$Y)
```
...
# Calcular Kappa
kappa_GLMd <- kappa2(cbind(predGLMd, testData_matriz_dico$Y))
...
# Exportar a Excel
write_xlsx(resultados_modelos_original, "C:/Users/karin/Desktop/MCD/TESIS/resultados_modelos_original_matriz.xlsx")
```
