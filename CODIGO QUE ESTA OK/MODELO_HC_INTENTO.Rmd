...
```{r}
featureMatrix_dicho<- cbind(featureMatrix_dicho, label=Datos_actualizados$PCR.Cov, equipo=Datos_actualizados$equipo)#1=neg,2=pos
```


```{r}
# Eliminar la columna "label" de featureMatrix_dicho
featureMatrix_dicho <- featureMatrix_dicho[, !colnames(featureMatrix_dicho) %in% "label"]

```

```{r}
table(featureMatrix_dicho$equipo)
```


```{r}
# Filtrar la matriz por aquellos que tienen equipo "HC"
featureMatrix_dicho_HC <- featureMatrix_dicho[featureMatrix_dicho$equipo == "HC", ]
...
trainIndexHC <- createDataPartition(featureMatrix_dicho_HC[, 18], p = 0.8, list = FALSE)

# Crear conjuntos de entrenamiento y prueba, asegurando que ambos son dataframes
trainDataHC <- as.data.frame(featureMatrix_dicho_HC[trainIndexHC, , drop = FALSE])
testDataHC <- as.data.frame(featureMatrix_dicho_HC[-trainIndexHC, , drop = FALSE])

# Verificar los nombres de columnas (ajusta si la columna no se llama 'covid')
colnames(trainDataHC)
...
```{r}
controlHC <- trainControl(method = "cv", number = 5)
...
GBM_model <- train(
  ...
  data = trainDataHC,            # Tu conjunto de entrenamiento
  method = "gbm",              # Método GBM
  trControl = controlHC,         # Control de entrenamiento
