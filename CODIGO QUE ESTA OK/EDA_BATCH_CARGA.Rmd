...
```{r}
save(Datos_filtrados, file = "metadata_high_int.rda")
...
```{r}
save(featureMatrix_numf, file = "featureMatrix_high_int.rda")
...
umap_v2 <- umap(featureMatrix_numf, n_neighbors = 5, metric = "cosine", min_dist = 0.1)
...
umap_coor3$equipo <- Datos_filtrados$equipo
...
Datos_filtrados <- Datos_filtrados[,c(2:7)] %>%
...
perform_pvca <- function(featureMatrix_numf, Datos_filtrados, factors_to_analyze) {
  print("Starting PVCA analysis...")
  
  # Check inputs
  if (!is.matrix(featureMatrix_numf) || !is.numeric(featureMatrix_numf)) {
    stop("featureMatrix_num must be a numeric matrix")
  }
  if (!is.data.frame(Datos_filtrados)) {
    stop("Datos_actualizados must be a data frame")
  }
  if (!all(factors_to_analyze %in% names(Datos_filtrados))) {
    stop("Not all factors_to_analyze are present in Datos_filtrados")
  }
  if (nrow(featureMatrix_numf) != nrow(Datos_filtrados)) {
  ...
  for (i in 1:n_pcs) {
    pc_data <- data.frame(PC = pca_result$x[, i], Datos_filtrados)
...
print("Running PVCA...")
results <- perform_pvca(featureMatrix_numf, Datos_filtrados, factors_to_analyze)
print("PVCA Results:")
print(results)
...
fviz_pca_ind(pca_result,
             label = "none", # No muestra las etiquetas de las muestras
             habillage = Datos_filtrados$equipo, # Variable para colorear
             addEllipses = TRUE, # Añade elipses de confianza
             ellipse.level = 0.95) # Nivel de confianza
...
featureMatrix_cov_pos <- featureMatrix_numf[indices_cov_pos, ]
Datos_actualizados_cov_pos <- Datos_filtrados[indices_cov_pos, ]
...
featureMatrix_cov_neg <- featureMatrix_numf[indices_cov_neg, ]
Datos_actualizados_cov_neg <- Datos_filtrados[indices_cov_neg, ]
...
featureMatrix_equipo <- featureMatrix_numf
...
combat_corrected_equipo_carga <- ComBat(
...
combat_corrected_equipo_carga <- t(combat_corrected_equipo_carga)

# Guardar la matriz corregida
write.csv(combat_corrected_equipo_carga, "corrected_featureMatrix.csv")

```


```{r}
save(combat_corrected_equipo_carga, file = "correccion_combat_equipo_high_int.rda")
...
perform_pvca <- function(combat_correctedf, Datos_filtrados, factors_to_analyze) {
  print("Starting PVCA analysis...")

  # Comprobaciones de entrada
  if (!is.matrix(combat_correctedf) || !is.numeric(combat_correctedf)) {
  ...
  if (nrow(combat_correctedf) != nrow(Datos_filtrados)) {
  ...
  pca_result <- prcomp(combat_correctedf, scale. = TRUE)
...
combatf_pvca_results <- perform_pvca(combat_correctedf, Datos_filtrados, factors_to_analyze)

print("PVCA Results after ComBat:")
print(combatf_pvca_results)

print("Creating plot for PVCA results after ComBat...")
plot_pvca(combatf_pvca_results)
...
# Ajustar parámetros directamente en la función umap
umap_v4 <- umap(combat_correctedf, n_neighbors = 15, metric = "cosine", min_dist = 0.1)
...
umap_coor4$equipo <- Datos_filtrados$equipo
...
combat_corrected_fecha_carga <- ComBat(
  dat = t(data_matrix),             # Matriz de intensidades con las muestras en filas
  batch = batch_info,            # Información de batch
  par.prior = TRUE,
  prior.plots = FALSE
...
```{r}
# Transponer de vuelta los datos corregidos
combat_corrected_fecha_carga <- t(combat_corrected_fecha_carga)

# Guardar la matriz corregida
write.csv(combat_corrected_fecha_carga, "corrected_featureMatrix.csv")

```


```{r}
save(combat_corrected_fecha_carga, file = "correccion_combat_fecha_high_int.rda")
...
```{r}
combat_corrected_fechaequipo <- ComBat(
...
# Ajustar parámetros directamente en la función umap
umap_v5 <- umap(combat_corrected2f, n_neighbors = 15, metric = "cosine", min_dist = 0.1)
...
umap_coor5$equipo <- Datos_filtrados$equipo
