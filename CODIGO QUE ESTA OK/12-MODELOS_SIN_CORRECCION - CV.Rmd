...
thr_filtered <- thr[colnames(trainData_matriz_dico[,1:43])]
```
...
```{r}
test_subset_matriz <- test_subset_matriz[, colnames(trainData_matriz_dico)[1:43]]
...
library(dplyr)
testData_matriz_dico<-as.data.frame(testData_matriz_dico)
testData_matriz_dico <- testData_matriz_dico %>% rename(Y = V44)
str(testData_matriz_dico$Y)
```
