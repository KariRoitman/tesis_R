...
Categ.Ciego_8_7.4,
Categ.Malbran.1.1.1,
Categ.Malbran.2.1.1<-Categ.Malbran.2.1.1[1:18,],
Malbran_3_4.1,
CostaRica.1.1,
CostaRica.3.1.1.1)
...
Espec.Union.Clin.Pos.Neg <- c(
                            ...
                            #Espectros.Malbran.24.covid,
                            #Espectros.Malbran.24.No.covid[1:4],
                            Espectros.Malb.2.24<-Espectros.Malb.2.24[1:18],
                            Espectros.Malb.3_4,
                            Espectros.CR1,
                            Espectros.CR3,
                             Espectros.CR4, #ciego
...
```{r}
#Extraigo los valores numéricos (NA cuando no son parseables)
featureMatrix<-as.data.frame(featureMatrix)
num <- suppressWarnings(as.numeric(as.character(featureMatrix$carga)))
featureMatrix<-as.data.frame(featureMatrix)
# 2) Recategorizo únicamente donde hay número
...
Quedan 228 espectros con cargas HIGH INT y NEG. Aclaracion: como costa rica no coloco el dato, en carga solo dice Cov.Pos o neg, no se incluyen en el análisis
...
# 9.1 Matriz de intensidades con label
...
plot_pca(pca_data, PC1, PC2, color_by = "equipo", shape_by = "covid",
         title = "PCA - PC1 y PC2 por equipo-sin otros virus y con cargas altas", usar_paleta_auto = TRUE)

```

```{r}
plot_pca(pca_data, PC2, PC3, color_by = "equipo", shape_by = "covid", 
         title = "PCA - PC2 vs PC3 por equipo y pos/neg-sin otros virus y con cargas altas", usar_paleta_auto = TRUE)
```

```{r}
plot_pca(pca_data, PC1, PC2, color_by = "fecha", shape_by = "covid",
         title = "PCA - PC1 vs PC2 por fecha-sin otros virus y con cargas altas",
...
plot_pca(pca_data, PC1, PC2, color_by = "covid", shape_by = "covid", 
         title = "PCA - PC1 vs PC2 COV POS/NEG-sin otros virus y con cargas altas", usar_paleta_auto = TRUE)
```


```{r}
plot_pca(pca_data, PC1, PC3, color_by = "covid", shape_by = "covid", 
         title = "PCA - PC1 vs PC3 COV POS/NEG-sin otros virus y con cargas altas", usar_paleta_auto = TRUE)
```
...
```{r}
UMAP_base <- UMAP_base + theme(legend.position = "right")
UMAP_ajustado <- UMAP_ajustado + theme(legend.position = "right")
...
combined_plot_UMAP <- combined_plot + 
  plot_annotation(title = "UMAP", tag_levels = "A") + 
...
```{r}
ggsave(
  filename = "combined_plot_UMAP_carga_covid.png",
...
combined_plot_tSNE_carga_covid <- (g1 | g2) +
  plot_annotation(
...
# Mostrar
print(combined_plot_tSNE_carga_covid)
...
# 
# # Agregar títulos a cada gráfico
# combined_plot_tSNE_carga_covid <- combined_plot + 
#   plot_annotation(
#     title = "UMAP",
#     tag_levels = "A"  # Etiquetas A, B, C, ...
#   ) + 
...
```{r}
ggsave(
  filename = "combined_plot_tSNE_carga_covid.png",
  plot = combined_plot_tSNE_carga_covid,
  width = 12,
  height = 10,
  dpi = 300
)
...
```{r}
plot_dendrogram <- function(data_matrix, labels_vector, titulo = "Dendrograma",
                            nombre_png = NULL, colores = NULL,
                            cex_labels = 0.4, cex_leyenda = 0.8,
                            method = "ward.D2", titulo_leyenda = NULL,
  ...
  if (is.null(colores)) {
    if (palette_type == "brewer") {
      colores <- if (n_levels <= 9) {
        RColorBrewer::brewer.pal(n_levels, "Set3")
      } else {
        colorRampPalette(RColorBrewer::brewer.pal(9, "Set3"))(n_levels)
      }
    } else {
...
plot_dendrogram(featureMatrix_num, COVID,
                ...
                nombre_png = "dendrograma_cc.png",

  cex_labels = 0.4,     # etiquetas de abajo más chicas
  cex_leyenda = 0.8      # leyenda normal
)
...
plot_dendrogram(featureMatrix_num[indices_cov_pos, ],
                ...
                nombre_png = "dendrograma_cc.pos.png")
...
plot_dendrogram(featureMatrix_num[indices_cov_neg, ],
                ...
                nombre_png = "dendrograma_cc.neg.png")
...
plot_dendrogram(featureMatrix_num, equipo,
                ...
                nombre_png = "dendrograma_cc.equipo.png")
...
plot_dendrogram(featureMatrix_num, fecha,
                ...
                nombre_png = "dendrograma_fecha_cc.png",
                              palette_type = "brewer",
...
```{r}
combat_corrected_fecha_cc<-combat_corrected_fecha
```

```{r}
combat_corrected_equipo_cc<-combat_corrected_equipo
```
